...
const ExplanationView = ({ view }) => {
  ...
  const topic =
    view === "topicExplanation" ? layer.topics[currentTopic - 1] : null;
  ...
  // ✅ Reset correctly when content changes
  useEffect(() => {
    setCurrentScene(1);
  ...
  return (
    <div className="glass-panel rounded-xl p-6 flex-1 flex flex-col gap-6">
      ...
      <button
        onClick={handleNext}
        className="glass-panel neon-border-cyan rounded-lg px-4 py-3 text-sm tracking-widest hover:scale-[1.02] transition-transform"
      >
        {currentScene < scenes.length
          ? "CONTINUE"
          : view === "topicExplanation"
