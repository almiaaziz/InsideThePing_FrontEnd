...
export const MissionResult = ({ tries, onContinue }) => {
  ...
  return (
    <div className="relative z-10 flex flex-col items-center gap-6 sm:gap-10 animate-fade-in">
      {/* Ping display */}
      <div className="flex flex-col items-center gap-2 sm:gap-4">
        ...
        <div
          className="text-6xl sm:text-8xl md:text-9xl font-display font-black text-mono"
          style={{
            color: currentColor,
      ...
      {showMessage && (
        <div className="flex flex-col items-center gap-4 sm:gap-6 animate-fade-in w-full">
          <div className="glass-panel neon-border-green rounded-2xl px-6 sm:px-10 py-4 sm:py-6 text-center">
            <h2 className="text-display text-lg sm:text-2xl tracking-wider glow-text-green mb-2">
              MISSION COMPLETE
            </h2>
          ...
          <div className="flex gap-3 sm:gap-4">
            {[
              { label: "XP EARNED", value: `+${xpGained}`, cls: "glow-text-cyan" },
              ...
              <div key={s.label} className="glass-panel rounded-xl px-4 sm:px-6 py-3 sm:py-4 text-center">
                <div className="text-[9px] sm:text-[10px] text-muted-foreground font-display tracking-widest mb-1">
                  {s.label}
                </div>
                <div className={`text-base sm:text-xl font-display font-bold ${s.cls}`}>
                  {s.value}
                </div>
              </div>
            ))}
          ...
          <button
            onClick={onContinue}
            className="glass-panel flex neon-border-cyan rounded-xl px-8 sm:px-10 py-2 sm:py-3 font-display text-xs sm:text-sm tracking-[0.3em] text-primary hover:scale-105 transition-transform"
          >
